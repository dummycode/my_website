<?php
    if(strcmp($_GET['api'], "false") == "0")
    {
        ...
        if(!$tokenIsValid[0])
        	...
        	echo "searching with user id=" . $tokenIsValid . "<br><br>";
        	echo apiListTripsEndingNearLocation($locationLatitude, $locationLongitude);
        }
        
    }
    function apiListTripsEndingNearLocation($locationLatitude, $locationLongitude)
    {
        ...
        if(mysqli_num_rows($tripsNearLocation) == 0)
            return "no trips ending near specified location, create one today";
       	$counter = 0;
       	$result = '';
        while($row = mysqli_fetch_array($tripsNearLocation))
        {
            if($counter === 0)
                $result = $row['id'] . " ending " . $row['distance'] . " miles away on " . $row['date'];
            else
                $result .= ',' . $row['id'] .  " ending " . $row['distance'] . " miles away on " . $row['date'];
            $counter = $counter + 1;
        }
